...
void	axes_draw(struct s_mlx *mlx)
{
	...
	axis = vec3_new(
			axes.m00 * AXIS_SIZE + AXIS_CENTER_X,
			axes.m01 * AXIS_SIZE + AXIS_CENTER_Y,
			0.0f);
	line_draw(mlx, origin, axis, RED);
	axis = vec3_new(
			axes.m10 * AXIS_SIZE + AXIS_CENTER_X,
			axes.m11 * AXIS_SIZE + AXIS_CENTER_Y,
			0.0f);
	line_draw(mlx, origin, axis, GREEN);
	axis = vec3_new(
			axes.m20 * AXIS_SIZE + AXIS_CENTER_X,
			axes.m21 * AXIS_SIZE + AXIS_CENTER_Y,
			0.0f);
	line_draw(mlx, origin, axis, BLUE);
}
