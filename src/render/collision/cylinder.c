...
/*   Updated: 2025/02/05 02:34:28 by maamine          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <render/collision.h>

static float	get_determinant(struct s_cylinder *cyl, t_vec3 ray_to_base, t_vec3 away_from_axis)
{
	float	determinant;
	float	radius;
	...
	determinant = vec3_length_sq(away_from_axis) * radius * radius - determinant;
	return (determinant);
}
		...
		float dist_front, struct s_cylinder *cyl, t_collision *coll)
{
	...
	if (height_front < 0)
	{
		coll->dist = dist_front + (inside * height_front / (height_front - height_back));
		coll->normal = vec3_scal_mul(cyl->axis, -1.0f);
	}
	else
	{
		coll->dist = dist_front + (inside * (height_front - cyl->height) / (height_front - height_back));
		coll->normal = cyl->axis;
	}
}

static bool		end_caps(t_vec3 values, struct s_ray *ray, struct s_cylinder *cylinder, t_collision *coll)
{
	float	height_back;
	float	height_front;
	float	dist_front;
	...
	inside = values.z;
	height_back = vec3_dot(cylinder->axis, vec3_sub(ray_collision(ray, dist_front + inside), cylinder->base.position));
	if ((height_back < 0 && height_front < 0) || (height_back > cylinder->height && height_front > cylinder->height))
		return (false);
	if (!coll)
		return (true);
...
static void		body(float dist, struct s_cylinder *cylinder, struct s_ray *ray, t_collision *coll)
{
	t_vec3	base_to_coll;
	t_vec3	offset;
	...
	offset = vec3_scal_mul(cylinder->axis, vec3_dot(base_to_coll, cylinder->axis));
	coll->normal = vec3_normalise(vec3_sub(base_to_coll, offset));
}

bool	cylinder_collide(struct s_ray *ray, struct s_cylinder *cylinder, t_collision *coll)
{
	...
	height = vec3_dot(cylinder->axis, vec3_sub(ray_collision(ray, dist), cylinder->base.position));
	if (height < 0 || height > cylinder->height)
	{
		determinant = 2 * sqrtf(determinant) / vec3_length_sq(away_from_axis);
		return (end_caps(vec3_new(dist, height, determinant), ray, cylinder, coll));
	}
	else
